...
module.exports = {
  ...
  async execute(interaction) {
    const isTarget =
      (interaction.isButton() && ["leavemsg_toggle", "leavemsg_open_modal", "leavemsg_open_message_modal"].includes(interaction.customId)) ||
      (interaction.isModalSubmit() && interaction.customId === "leavemsg_modal") ||
      (interaction.isChannelSelectMenu() && interaction.customId === "leavemsg_select_channel");
    ...
    if (interaction.isButton() && interaction.customId === "leavemsg_open_modal") {
      const channelSelect = new ChannelSelectMenuBuilder()
        .setCustomId("leavemsg_select_channel")
      ...
      const openMessageModalRow = new ActionRowBuilder().addComponents(
        new ButtonBuilder()
          .setCustomId("leavemsg_open_message_modal")
          .setLabel("Leaveメッセージ本文を編集")
    ...
    if (interaction.isButton() && interaction.customId === "leavemsg_open_message_modal") {
      const modal = new ModalBuilder().setCustomId("leavemsg_modal").setTitle("Leaveメッセージ本文設定");
    ...
    if (interaction.isChannelSelectMenu() && interaction.customId === "leavemsg_select_channel") {
