...
const command = {
    ...
    async execute(interaction: ChatInputCommandInteraction) {
        const sendGameMessage = async (embed: EmbedBuilder, components: ActionRowBuilder<ButtonBuilder>[] = []) => {
        ...
        const replyError = async (content: string) => {
            ...
            if (interaction.deferred || interaction.replied) {
                await interaction.followUp({ embeds: [embed], flags: ["Ephemeral"] as const });
            } else {
                await interaction.reply({ embeds: [embed], flags: ["Ephemeral"] as const });
            }
        };
        ...
        if (mode === "guild") {
            ...
            const embed = new EmbedBuilder()
                ...
                .setDescription(`${user1}が対戦相手を募集しています！\n下のボタンを押してゲームに参戦してください。`)
                .setColor(0x9b59b6)
                .setTimestamp(new Date());
            ...
            collector.on("collect", async (i) => {
                if (i.user.id === user1.id) {
                    await i.reply({ content: "❌ 自分自身と対戦することはできません！", ephemeral: true });
                    return;
                }
            ...
            collector.on("end", async (_, reason) => {
                if (reason !== "matched") {
                    const timeoutEmbed = new EmbedBuilder()
                        .setDescription("⏳ 誰も来なかったため、募集がキャンセルされました。")
                        .setColor(0x95a5a6);
...
async function startGame(
    ...
    await message.edit({
        embeds: [buildGameEmbed("順番に穴を刺してください。ハズレを引くと黒ひげが飛び出します！")],
    ...
    const handleAiAction = async () => {
        ...
        const remainingHoles = Array.from({ length: totalHoles }, (_, i) => i).filter(
            (h) => !safeHoles.includes(h)
        );
        const aiChoice = remainingHoles[Math.floor(Math.random() * remainingHoles.length)];

        if (aiChoice === loseHole) {
            collector.stop("gameover");
            const winEmbed = new EmbedBuilder()
                .setAuthor({ name: "💥 ドカーン！ゲーム終了", iconURL: SUCCESS_ICON_URL })
                .setDescription(`🤖 AIがハズレの穴を刺しました！\n\n🏆 **勝者: ${player1}**`)
                .setColor(0x57f287)
                .setTimestamp(new Date());
            await message.edit({ embeds: [winEmbed], components: [] }).catch(() => null);
        } else {
    ...
    collector.on("collect", async (i) => {
        ...
        if (i.user.id !== activePlayer.id) {
            await i.reply({ content: "❌ あなたのターンではありません！", ephemeral: true });
            return;
        }
        ...
        if (selected === loseHole) {
            ...
            const winner = turn === 1 ? (isAi ? "AI🤖" : (player2?.username ?? "Unknown")) : player1.username;

            const loseEmbed = new EmbedBuilder()
                .setAuthor({ name: "💥 ドカーン！ゲーム終了", iconURL: ERROR_ICON_URL })
                .setDescription(`${i.user} がハズレを刺してしまいました！黒ひげが飛び出します！\n\n🏆 **勝者: ${winner}**`)
                .setColor(0xed4245)
                .setTimestamp(new Date());

            await message.edit({ embeds: [loseEmbed], components: [] }).catch(() => null);
        } else {
            ...
            if (isAi) {
                ...
                await message.edit({ embeds: [aiThinkEmbed], components: buildButtons() }).catch(() => null);
    ...
    collector.on("end", async (_, reason) => {
        if (reason === "time") {
            const timeoutEmbed = new EmbedBuilder()
                .setDescription("⏳ プレイヤーの操作が無かったため、時間切れで終了しました。")
                .setColor(0x95a5a6);
