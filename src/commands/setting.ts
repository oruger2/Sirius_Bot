...
const parseSettingsRow = (row: RowDataPacket): GuildSettings => {
	return {
		joinMessage: safeParseSetting(
			row.setting_JoinMessage,
			DEFAULT_GUILD_SETTINGS.joinMessage,
		),
		leaveMessage: safeParseSetting(
			row.setting_LeaveMessage,
			DEFAULT_GUILD_SETTINGS.leaveMessage,
		),
		spamBlock: safeParseSetting(
			row.setting_SpamBlock,
			DEFAULT_GUILD_SETTINGS.spamBlock,
		),
		autoReaction: safeParseSetting(
			row.setting_AutoReaction,
			DEFAULT_GUILD_SETTINGS.autoReaction,
		),
		shortLinkBlock: safeParseSetting(
			row.setting_ShortLinkBlock,
			DEFAULT_GUILD_SETTINGS.shortLinkBlock,
		),
		inviteLinkBlock: safeParseSetting(
			row.setting_InviteLinkBlock,
			DEFAULT_GUILD_SETTINGS.inviteLinkBlock,
		),
	};
};
...
const buildSettingsComponents = (
	...
	const navButtons = [
		...
		new ButtonBuilder()
			.setCustomId(
				`setting:page:${guildId}:${Math.min(page + 1, SETTINGS_PAGES.length - 1)}`,
			)
			.setLabel("次へ")
			.setStyle(ButtonStyle.Secondary)
...
export const handleSettingButtonInteraction = async (
	...
	if (parsed.action === "toggle") {
		...
		const page =
			interaction.customId.includes("joinMessage") ||
				...
				.addComponents(
					new ActionRowBuilder<TextInputBuilder>().addComponents(
						new TextInputBuilder()
							...
							.setValue(
								`${settings.spamBlock.timeoutSeconds} ${settings.spamBlock.deleteMessageSeconds}`,
							),
					),
				);
...
export const handleSettingModalInteraction = async (
	...
	if (field === "joinMessage" || field === "leaveMessage") {
		const channelInput = interaction.fields
			.getTextInputValue("setting-channel")
			.trim();
		const messageValue =
			interaction.fields.getTextInputValue("setting-message");
		...
		const timeoutParts = interaction.fields
		...
		nextSettings.autoReaction.emojis = interaction.fields
